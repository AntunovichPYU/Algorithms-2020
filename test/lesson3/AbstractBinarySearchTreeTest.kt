...
abstract class AbstractBinarySearchTreeTest {
    ...
    protected fun doRemoveTest() {
        ...
        assertFalse(create().remove(0))
        val controlSet = mutableSetOf<Int>()
        controlSet.addAll(arrayOf(1, 2, 5, 4, 7, 35, 64, 54))
        var toRemove = 4
        println("Initial set: $controlSet")
        val binarySet = create()
        for (element in controlSet) {
    ...
    protected fun doIteratorTest() {
        ...
        assertFailsWith<NoSuchElementException> { create().iterator().next() }
        val controlSet = TreeSet<Int>()
        controlSet.addAll(arrayOf(1, 2, 4, 3, 54, 23, 34))
        println("Control set: $controlSet")
        val binarySet = create()
        assertFalse(
            binarySet.iterator().hasNext(),
    ...
    protected fun doIteratorRemoveTest() {
        ...
        assertFailsWith<IllegalStateException> { create().iterator().remove() }
        val controlSet = TreeSet<Int>()
        controlSet.addAll(arrayOf(1, 3, 2, 4, 66, 78, 54))
        var toRemove = 78
        println("Initial set: $controlSet")
        val binarySet = create()
        for (element in controlSet) {
